...
const Panel = memo(function Panel({ path, active }: { path: string; active: boolean }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const showScrollToTop = el.scrollTop > SCROLL_THRESHOLD;
      if (active) setScrollButton(
        el,
        !sticky,
  ...
  // 后续流式增长仍走平滑 follow。
  // 只在首次有内容时 arm，后续切换不再触发，保持滚动位置。
  const hasArmedOnce = useRef(false);
  useLayoutEffect(() => {
    if (active && items.length > 0 && !hasArmedOnce.current) {
      bottomScroll.checkSticky(); // 先刷新 sticky 状态
      bottomScroll.armInstantLanding();
      hasArmedOnce.current = true;
    }
  }, [active, bottomScroll, items.length]);
...
function setScrollButton(
  ...
  scrollToTop: (() => void) | null = null,
) {
  _scrollBtn.el = el;
  _scrollBtn.visible = visible;
  _scrollBtn.showScrollToTop = showScrollToTop;
  _scrollBtn.scrollToBottom = scrollToBottom;
  _scrollBtn.scrollToTop = scrollToTop;
...
function ScrollToBottomBtn() {
  ...
  useEffect(() => {
    const update = () => {
      setVisible(_scrollBtn.visible);
      setShowScrollToTop(_scrollBtn.showScrollToTop);
    };
  ...
  return (
    <>
      {showScrollToTop && (
        <button className={styles.scrollToTopFab} onClick={() => {
          _scrollBtn.scrollToTop?.();
        }}>
