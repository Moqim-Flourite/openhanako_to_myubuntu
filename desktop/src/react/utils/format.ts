...
// ── 自定义：全局代码块复制按钮观察器 ──
let _globalCopyObserverStarted = false;

/**
 * 启动全局 MutationObserver，自动给新增的 <pre> 块注入复制按钮。
 * 只需调用一次，通常在应用初始化时。
 */
export function startGlobalCopyButtonObserver(): void {
  if (_globalCopyObserverStarted) return;
  _globalCopyObserverStarted = true;

  function processNode(node: Node): void {
    if (!(node instanceof HTMLElement)) return;
    if (node.tagName === 'PRE') {
      injectCopyButtons(node.parentElement || node);
    } else if (node.querySelector?.('pre')) {
  ...
  if (document.body) {
    for (const pre of document.body.querySelectorAll('pre')) {
      injectCopyButtons(pre.parentElement || pre);
    }
  }
