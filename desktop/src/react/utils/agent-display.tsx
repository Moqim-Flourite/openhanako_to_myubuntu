...
export function buildAgentDisplayMap(agents: Agent[]): Map<string, Agent> {
  ...
  for (const agent of agents) {
    ...
    // name 映射：不覆盖已有条目，防止不同 agent 的 name 冲突
    if (agent.name && !map.has(agent.name)) map.set(agent.name, agent);
    if (agent.displayName && agent.displayName !== agent.name && !map.has(agent.displayName)) map.set(agent.displayName, agent);
  }
  return map;
}
