...
interface Buffer {
  ...
  /** 缓存的 tool 事件，和文本一起批量 flush，避免并发工具调用时频繁触发 React 重渲染 */
  pendingToolEvents: Array<{ type: 'tool_start' | 'tool_end'; msg: any }>;
}
...
class StreamBufferManager {
  ...
  private hasTurnState(buf: Buffer): boolean {
    return !!(
      ...
      buf.cardAttrs ||
      buf.cardDescAcc ||
      buf.pendingToolEvents.length > 0
    );
  }
  ...
  private flush(buf: Buffer): void {
    ...
    this.updateTargetMessage(buf, (msg) => {
      ...
      // ── 批量处理缓存的 tool 事件（避免并发工具调用时每个事件单独触发 store 更新） ──
      if (buf.pendingToolEvents.length > 0) {
        const events = buf.pendingToolEvents.splice(0);
          ...
          if (ev.type === 'tool_start') {
            ...
            if (lastTg >= 0 && blocks[lastTg].type === 'tool_group') {
              ...
              if (tg.tools.some(t => !t.done)) {
                blocks[lastTg] = {
                  ...tg,
                  tools: [...tg.tools, { name: ev.msg.name, args: ev.msg.args, done: false, success: false }],
                };
                continue;
              }
            }
            blocks.push({
              type: 'tool_group',
              tools: [{ name: ev.msg.name, args: ev.msg.args, done: false, success: false }],
              collapsed: false,
            });
            ...
            for (let i = blocks.length - 1; i >= 0; i--) {
              ...
              const toolIdx = tg.tools.findIndex(t => t.name === ev.msg.name && !t.done);
              if (toolIdx >= 0) {
                const tools = [...tg.tools];
                tools[toolIdx] = { ...tools[toolIdx], done: true, success: !!ev.msg.success, details: ev.msg.details };
  ...
  handle(msg: any): void {
    ...
    switch (msg.type) {
      ...
      case 'tool_start':
        // 并发工具调用时事件频率高，缓存到 pendingToolEvents，随下次 flush 一起批量写入 store
        this.ensureMessage(buf);
        buf.pendingToolEvents.push({ type: 'tool_start', msg });
        this.scheduleFlush(buf);
        break;

      case 'tool_end':
        buf.pendingToolEvents.push({ type: 'tool_end', msg });
        this.scheduleFlush(buf);
        break;
