...
export function captureChannelSelection(channelId: string, fallbackAnchorRect?: FloatingAnchorRect): void {
  const sel = window.getSelection();
  const text = sel?.toString().trim();
  if (!sel || !text || sel.rangeCount === 0) {
    clearSelection({ sourceKind: 'chat', sourceSessionPath: channelId });
    return;
  }
  ...
  // Channel messages use composite key: timestamp+sender
  // Look up from channelMessages store
  const messages = useStore.getState().channelMessages;
  ...
  const quotedSelection: QuotedSelection = {
    ...
    sourceTitle: `${msg.sender}`,
    sourceKind: 'chat',
    sourceSessionPath: channelId,
...
function captureDocumentChatSelection(target: Document, fallbackAnchorRect?: FloatingAnchorRect): void {
  ...
  // Check if this is a channel selection
  const selectionType = anchorRoot.dataset.selectionType;
  if (selectionType === 'channel') {
    captureChannelSelection(sessionPath, fallbackAnchorRect);
  } else {
    captureChatSelection(sessionPath, fallbackAnchorRect);
  }
}
