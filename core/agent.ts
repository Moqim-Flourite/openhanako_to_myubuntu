...
export class Agent {
  ...
  async init(log: (msg?: string) => void = () => {}, sharedModels: any = {}, resolveModel = null) {
    ...
    this._stageFilesTool = createStageFilesTool({
      ...
      getSessionPath: () => this._cb?.getCurrentSessionPath?.(),
      sendBridgeMedia: ({ platform, chatId, agentId, isGroup, mediaItem }) => {
        const engine = this._cb?.getEngine?.();
        const bridgeManager = engine?._hubCallbacks?.hub?.bridgeManager;
        if (!bridgeManager?.sendMediaItem) return;
        return bridgeManager.sendMediaItem(platform, chatId, mediaItem, agentId, { isGroup });
      },
      resolveBridgeOwnerChatId: (platform) => {
        const engine = this._cb?.getEngine?.();
        const bridgeSessionManager = engine?.bridgeSessionManager;
        if (!bridgeSessionManager) return null;
        const index = bridgeSessionManager.readIndex?.(this) || {};
        // index key 格式: qq_dm_{chatId}@{agentId}，按 platform 前缀匹配
        const prefix = `${platform}_dm_`;
        for (const [key, entry] of Object.entries(index)) {
          if (!key.startsWith(prefix)) continue;
          // 从 key 提取 chatId: qq_dm_{chatId}@agent → chatId
          const chatId = key.slice(prefix.length).split("@")[0];
