...
function resolveDelegateMode(mode) {
  if (mode === "full" || mode === "safe" || mode === "read-only") return mode;
  return "read-only";
}
...
export function createDelegateTool(deps) {
  return withToolMeta({
    ...
    parameters: Type.Object({
      ...
      mode: Type.Optional(
        Type.Union([
          ...
          Type.Literal("safe"),
          Type.Literal("full"),
        ], { description: "子代理工具权限模式：read-only（只读）/ safe（安全）/ full（完全）" })
      ),
    }),
    ...
    execute: async (_toolCallId, params, signal) => {
      if (activeCount >= MAX_CONCURRENT) {
        return {
          ...
          details: { mode: params.mode || "read-only" },
        };
      }

      const mode = resolveDelegateMode(params.mode);
      ...
      try {
        const result = await deps.executeIsolated(
          getDelegatePreamble() + params.task,
          {
            ...
            toolFilter: mode === "full" ? undefined : DELEGATE_CUSTOM_TOOLS,
            builtinFilter: mode === "full" ? undefined : deps.readOnlyBuiltinTools,
        ...
        if (result.error) {
          return {
            ...
            details: { mode, error: result.error },
          };
        }
        return {
    ...
    tags: ["agent", "delegate", "sub-agent", "orchestration"],
  });
}
