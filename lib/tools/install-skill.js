...
import { withToolMeta } from "./tool-meta.js";
import { requestToolConfirmation } from "./tool-confirmation.js";
 ...
 */
export function createInstallSkillTool({ agentDir, getConfig, resolveUtilityConfig, onInstalled, getConfirmStore, getSessionPath, emitEvent }) {
  return withToolMeta({
    name: "install_skill",
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (github_url?.trim()) {
        ...
        const installConfirm = await requestToolConfirmation({
          confirmStore: getConfirmStore?.(),
          emitEvent,
          sessionPath: getSessionPath?.() || null,
          toolName: "install_skill",
          action: "install_from_github",
          label: t("toolDef.installSkill.label"),
          description: "Allow the agent to install a skill from a GitHub repository into learned-skills.",
          payload: { source: "github", owner, repo, path: fetchedFrom, skillName: name, stars, reason },
        });
        if (installConfirm.action !== "confirmed") {
          return {
            content: [{ type: "text", text: installConfirm.action === "timeout" ? "Skill installation confirmation timed out" : "Skill installation was cancelled by user" }],
            details: { cancelled: true, source: "github", skillName: name },
          };
        }
      ...
      const installConfirm = await requestToolConfirmation({
        ...
        sessionPath: getSessionPath?.() || null,
        toolName: "install_skill",
        action: "install_from_content",
        label: t("toolDef.installSkill.label"),
        description: "Allow the agent to install provided skill content into learned-skills.",
        payload: { source: "content", skillName: name, reason },
      });
      if (installConfirm.action !== "confirmed") {
        return {
          content: [{ type: "text", text: installConfirm.action === "timeout" ? "Skill installation confirmation timed out" : "Skill installation was cancelled by user" }],
          details: { cancelled: true, source: "content", skillName: name },
        };
      }
    ...
    tags: ["skills", "install", "learning", "confirmation"],
  });
}
