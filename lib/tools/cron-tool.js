...
export function createCronTool(cronStore, { autoApprove = false, getAutoApprove, confirmStore, emitEvent, getSessionPath } = {}) {
  return withToolMeta({
        ...
        case "add": {
          ...
          if (confirmStore) {
            const result = await requestToolConfirmation({
              ...
              action: "add",
              label: t("toolDef.cron.label"),
              description: "Allow the agent to create a scheduled task.",
              payload: jobData,
            });
            ...
            if (result.action === "confirmed") {
              ...
              return {
                ...
                details: { action: "added", job, jobs: cronStore.listJobs(), confirmed: true },
              };
            }
            return {
              content: [{ type: "text", text: result.action === "rejected" ? t("error.cronRejected", { label }) : t("error.cronTimeout", { label }) }],
              details: { action: "cancelled", jobs: cronStore.listJobs(), confirmed: false },
            };
          }
    ...
    tags: ["cron", "schedule", "automation", "confirmation"],
  });
}
