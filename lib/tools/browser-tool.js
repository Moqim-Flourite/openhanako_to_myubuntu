...
export function createBrowserTool(opts = {}) {
  ...
  const getConfirmStore = opts.getConfirmStore || (() => null);
  ...
  return withToolMeta({
      ...
      try {
        switch (params.action) {
          ...
          case "navigate": {
            ...
            const navConfirm = await requestToolConfirmation({
              ...
              description: "Allow the agent to open an external web page in the embedded browser.",
              payload: { url: params.url },
            });
            if (navConfirm.action !== "confirmed") {
              return err(navConfirm.action === "timeout"
                ? "Browser navigation confirmation timed out"
                : "Browser navigation was cancelled by user");
            }
            ...
            return ok(
              ...
              { action: "navigate", ...await statusFields(), title: result.title, confirmed: true },
            );
          }
          ...
          case "evaluate": {
            ...
            const evalConfirm = await requestToolConfirmation({
              confirmStore: getConfirmStore(),
              emitEvent,
              sessionPath: getSessionPath(),
              toolName: "browser",
              action: "evaluate",
              label: "Browser script evaluation",
              description: "Allow the agent to execute JavaScript inside the current page context.",
              payload: { expression: params.expression.slice(0, 500) },
            });
            if (evalConfirm.action !== "confirmed") {
              return err(evalConfirm.action === "timeout"
                ? "Browser evaluate confirmation timed out"
                : "Browser evaluate was cancelled by user");
            }
            ...
            const truncated = result.length > 8000
              ...
              : result;
            return ok(truncated, { action: "evaluate", ...await statusFields(), confirmed: true });
          }
    ...
    tags: ["browser", "interactive", "web", "ui"],
  });
}
