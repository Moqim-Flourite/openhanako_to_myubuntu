 ...
 * @returns {Array<{sender: string, timestamp: string, body: string}>}
 */
export function getRecentMessages(filePath, count = 10, selfName, offset = 0) {
  if (!fs.existsSync(filePath)) return [];
  ...
  // 支持分页：offset 从末尾开始计算，offset=0 表示最新消息
  const start = Math.max(0, filtered.length - offset - count);
