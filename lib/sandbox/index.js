...
export function createSandboxedTools(cwd, customTools, { agentDir, workspace, hanakoHome, mode }) {
  log.log(`createSandboxedTools: cwd=${cwd}, mode=${mode}, agentDir=${agentDir}, workspace=${workspace || "none"}`);

  const policy = deriveSandboxPolicy({ agentDir, workspace, hanakoHome, mode });
  log.log(`createSandboxedTools: policy mode=${policy.mode}, writable=${policy.writablePaths?.length || 0}, protected=${policy.protectedPaths?.length || 0}, deny=${policy.denyReadPaths?.length || 0}`);
  ...
  if (policy.mode === "full-access") {
    log.log(`createSandboxedTools: full-access mode, no sandbox wrapping`);
  ...
  log.log(`createSandboxedTools: standard mode, platform=${platform}`);

  // Windows: PathGuard 包装生效，bash 用自定义 exec（避免 detached 导致空输出）
  if (platform === "win32-full-access") {
    log.log(`createSandboxedTools: Windows platform, using win32-exec`);
  ...
  if (!checkAvailability(platform)) {
    log.error(`createSandboxedTools: sandbox tool not available for platform=${platform}`);
  ...
  log.log(`createSandboxedTools: sandbox exec created for ${platform}`);
