...
const log = createModuleLogger("path-guard");
 ...
 */
  check(absolutePath, operation) {
    if (this._fullAccess) {
      log.log(`check: full-access mode, ${operation} on ${absolutePath} → allowed`);
      return { allowed: true };
    }
    ...
    if (allowed) {
      log.log(`check: ${operation} on ${absolutePath} → allowed (level=${level})`);
      return { allowed: true };
    }
    ...
    const reason = t("sandbox.denied", { op: opLabel, path: resolved, level });
    log.warn(`check: ${operation} on ${absolutePath} → DENIED (level=${level}, reason=${reason})`);
    return {
      allowed: false,
      reason,
    };
