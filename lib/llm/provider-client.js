...
export function normalizeProviderBaseUrl(baseUrl) {
  ...
  return {
    raw,
    disableAppend: hasTrailingHash(raw),
    normalized: stripTrailingHash(raw),
  };
}

export function buildApiEndpoint(baseUrl, api) {
  const { raw, disableAppend, normalized } = normalizeProviderBaseUrl(baseUrl);
  let endpoint = normalized;

  if (api === "openai-completions") {
    if (/\/chat\/completions$/i.test(normalized)) {
      endpoint = normalized;
    } else if (/\/(models|responses|messages)$/i.test(normalized)) {
      endpoint = normalized.replace(/\/(models|responses|messages)$/i, "/chat/completions");
    } else if (/\/v\d+$/i.test(normalized)) {
      endpoint = `${normalized}/chat/completions`;
    } else {
      endpoint = disableAppend ? normalized : `${normalized}/v1/chat/completions`;
    }
  ...
  if (api === "anthropic-messages") {
    if (/\/messages$/i.test(normalized)) {
      endpoint = normalized;
    } else if (/\/anthropic$/i.test(normalized)) {
      endpoint = disableAppend ? normalized : `${normalized}/v1/messages`;
    } else if (/\/v\d+$/i.test(normalized)) {
      endpoint = `${normalized}/messages`;
    } else {
      endpoint = disableAppend ? normalized : `${normalized}/v1/messages`;
    }
    debugProviderClient("buildApiEndpoint", { api, raw, normalized, disableAppend, endpoint });
  ...
  if (api === "openai-codex-responses" || api === "openai-responses") {
    if (/\/responses$/i.test(normalized)) {
      endpoint = normalized;
    } else if (/\/(chat\/completions|models|messages)$/i.test(normalized)) {
      endpoint = normalized.replace(/\/(chat\/completions|models|messages)$/i, "/responses");
    } else if (/\/v\d+$/i.test(normalized)) {
      endpoint = `${normalized}/responses`;
    } else {
      endpoint = disableAppend ? normalized : `${normalized}/v1/responses`;
    }
    debugProviderClient("buildApiEndpoint", { api, raw, normalized, disableAppend, endpoint });
    return endpoint;
  }
...
export function extractProviderBaseOrigin(fullUrl) {
  try {
    const url = new URL(String(fullUrl || "").trim());
    const path = url.pathname || "";
    const match = path.match(/\/v\d+(?=\/|$)/i);
    const prefix = match ? path.slice(0, match.index) : "";
    return `${url.protocol}//${url.host}${prefix}`.replace(/\/+$/, "");
  } catch {
    return stripTrailingHash(fullUrl || "");
  }
}

export function buildModelEndpoints(baseUrl, api = "openai-completions") {
  const { raw, disableAppend, normalized } = normalizeProviderBaseUrl(baseUrl);
  let urls = [];

  if (!normalized) {
    debugProviderClient("buildModelEndpoints", { raw, normalized, disableAppend, api, urls });
    return urls;
  }
  if (disableAppend) {
    urls = [normalized];
  ...
  const endpoint = buildApiEndpoint(normalized, api);
  const baseOrigin = extractProviderBaseOrigin(endpoint);

  if (api === "anthropic-messages") {
    urls = [`${baseOrigin}/v1/models`];
    debugProviderClient("buildModelEndpoints", { raw, normalized, disableAppend, api, endpoint, baseOrigin, urls });
    return urls;
  }

  const primary = `${baseOrigin}/v1/models`;
  const fallback = `${baseOrigin}/models`;
  urls = primary === fallback ? [primary] : [primary, fallback];
  debugProviderClient("buildModelEndpoints", { raw, normalized, disableAppend, api, endpoint, baseOrigin, urls });
  return urls;
}
...
function extractOpenAIText(data) {
  ...
  const reasoning = contentToText(message?.reasoning_content).trim();
