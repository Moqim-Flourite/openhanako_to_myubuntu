...
| 平台检测 | ✅ 已完成 | 原项目已实现 |
| 构建系统 | ✅ 已完成 | AppImage + deb 打包 |
| Electron 主进程 | ✅ 已完成 | Linux 平台适配 |
| Bridge 文件投递 | ✅ 已完成 | phone session 跨平台文件发送 |
| 系统托盘 | 📋 待开始 | GNOME 桌面集成 |
| 自动更新 | 📋 待开始 | Linux 更新机制 |
...
- **频道输入框引用片段**：ChannelInput 支持展示引用片段，发送时自动拼接引用文本，发送后自动清除引用
- **频道滚动位置修复**：修复切换频道时滚动位置读取到已更新 DOM 的 bug，改用 scroll listener 持续追踪

### 历史更新（v0.330.0 - 2026-06-26）
...
- AppImage: 见 [Releases](https://github.com/Moqim-Flourite/openhanako_to_myubuntu/releases)
- 或从本地构建：`npm run dist:linux`
...
{
  "scripts": {
    "dist:linux": "npm run build:client && npm run build:server && electron-builder --linux AppImage deb"
  },
  "build": {
    "linux": {
      "target": ["AppImage", "deb"],
...
`core/agent.js` 中 `resolveBridgeOwnerChatId` 使用 `engine.bridgeSessionManager.readIndex` 替代错误的 `bridgeManager.readIndex`，确保 phone session 能正确解析 bridge index 并发送文件到 QQ 等平台。
